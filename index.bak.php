...
if ( $USER->instructor ) {
    ...
    if ($handledRoster) {
        ...
        $response = LTI::getContextMemberships($ROSTER->id, $ROSTER->url, $key, LTIX::decrypt_secret($encryptedSecret), true);
        ...
        //public static function getContextMemberships($membershipsid, $membershipsurl, $key_key, $secret, $groups=false, $signature=false, &$debug_log=false) {
            ...
            $membershipsid = htmlspecialchars($ROSTER->id);
            ...
            $acc_req = OAuthRequest::from_consumer_and_token($test_consumer, $test_token, "POST", $ROSTER->url, $parameters);
            $acc_req->sign_request($hmac_method, $test_consumer, $test_token);
    
            $header = $acc_req->to_header();
            $header .= PHP_EOL . "Content-Type: " . $content_type . PHP_EOL;

            var_dump(Net::doBody($ROSTER->url, "POST", $body, $header));
    
            //return self::parseContextMembershipsResponse(Net::doBody($ROSTER->url, "POST", $body, $header));
/*    
